...
def compute_neuron_composition(model, layer, zero_diag=False):
    ...
    in_in_cos = einops.einsum(W_in, W_in[layer, :, :], f"l n d, m d -> m l n")
    in_out_cos = einops.einsum(W_out, W_in[layer, :, :], f"l n d, m d -> m l n")
    out_in_cos = einops.einsum(W_in, W_out[layer, :, :], f"l n d, m d -> m l n")
    out_out_cos = einops.einsum(W_out, W_out[layer, :, :], f"l n d, m d -> m l n")
...
def compute_neuron_statistics(model):
    ...
    #     [range(layers), range(4 * d_model)], names=["layer", "neuron_ix"]
    # )
    index = pd.MultiIndex.from_product(
        [
            range(layers),
            range(W_in_norms.shape[1]),
        ],
...
def run_weight_summary(args, model):
    save_path = os.path.join(args.save_path, args.model, "weights")
    ...
    for layer in tqdm.tqdm(range(model.cfg.n_layers)):
        k_comp, q_comp, v_comp, o_comp = compute_attention_composition(model, layer)
        k_comps.append(k_comp)
        q_comps.append(q_comp)
        v_comps.append(v_comp)
        o_comps.append(o_comp)
    ...
    torch.save(k_comps.to(torch.float16), os.path.join(save_path, "k_comps.pt"))
    torch.save(q_comps.to(torch.float16), os.path.join(save_path, "q_comps.pt"))
    torch.save(v_comps.to(torch.float16), os.path.join(save_path, "v_comps.pt"))
    torch.save(o_comps.to(torch.float16), os.path.join(save_path, "o_comps.pt"))
    ...
    for layer in tqdm.tqdm(range(model.cfg.n_layers)):
        ...
        for comp_type, comp_score in zip(vocab_composition_types, layer_comps):
            ...
            bottom, bottom_ix = torch.topk(comp_score, 100, dim=1, largest=False)
    ...
    vocab_comp_dict = {
        comp_type: {
            data_type: torch.stack(data_dict, dim=0)
    ...
    for layer in tqdm.tqdm(range(model.cfg.n_layers)):
        ...
        for comp_type, comp_score in zip(neuron_composition_types, layer_comps):
            ...
            bottom, bottom_ix = torch.topk(comp_score, 20, dim=1, largest=False)
...
def run_full_weight_analysis(
    model, causal_only=False, save_precision=8, save_path="results/weights"
):
    ...
    for layer in range(model.cfg.n_layers):
        # neuron composition
        in_in_cos, in_out_cos, out_out_cos = compute_neuron_composition(model, layer)

        torch.save(
            adjust_precision(
                in_in_cos, save_precision, per_channel=False, cos_sim=True
            ),
            os.path.join(neuron_cos_path, f"in_in_cos_{layer}.pt"),
        )
        torch.save(
            adjust_precision(
                in_out_cos, save_precision, per_channel=False, cos_sim=True
            ),
            os.path.join(neuron_cos_path, f"in_out_cos_{layer}.pt"),
        )
        torch.save(
            adjust_precision(
                out_out_cos, save_precision, per_channel=False, cos_sim=True
            ),
        ...
        torch.save(
            adjust_precision(in_E_cos, save_precision, per_channel=False, cos_sim=True),
            os.path.join(vocab_cos_path, f"in_E_cos_{layer}.pt"),
        )
        torch.save(
            adjust_precision(in_U_cos, save_precision, per_channel=False, cos_sim=True),
            os.path.join(vocab_cos_path, f"in_U_cos_{layer}.pt"),
        )
        torch.save(
            adjust_precision(
                out_E_cos, save_precision, per_channel=False, cos_sim=True
            ),
            os.path.join(vocab_cos_path, f"out_E_cos_{layer}.pt"),
        )
        torch.save(
            adjust_precision(
                out_U_cos, save_precision, per_channel=False, cos_sim=True
            ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", default="pythia-70m", help="Name of model from TransformerLens"
    )
    ...
    parser.add_argument("--save_path", default="summary_data")
    parser.add_argument("--compute_full_stats", action="store_true", default=False)
