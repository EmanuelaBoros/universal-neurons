...
def compute_moments_from_binned_data(bin_edges, bin_counts):
    ...
    variance = (((-mean[:, :, None] + bin_centers) ** 2) * bin_counts).sum(
        axis=-1
    ) / total_frequency
    skewness = (((-mean[:, :, None] + bin_centers) ** 3) * bin_counts).sum(axis=-1) / (
        total_frequency * (variance**1.5)
    )
    kurtosis = (((-mean[:, :, None] + bin_centers) ** 4) * bin_counts).sum(axis=-1) / (
        total_frequency * (variance**2)
    )
    return mean, variance, skewness, kurtosis


def make_pile_subset_distribution_activation_summary_df(
    dataset_summaries, bin_edges, include_all=False
):
    if include_all:
        full_distribution_bin_count = sum(
            dataset_summaries[k]["neuron_bin_counts"] for k in dataset_summaries.keys()
        )
...
def get_activation_sparsity_df(dataset_summaries, bin_edges):
    ...
    for dataset_name, dataset_summary in dataset_summaries.items():
        ...
        sparsity = dataset_summary["neuron_bin_counts"][:, :, zero_bin:].sum(
            axis=-1
        ) / dataset_summary["neuron_bin_counts"].sum(axis=-1)
