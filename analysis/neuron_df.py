...
def make_neuron_stat_df(model_name):
    ...
    print(f"Loading summaries for {model_family}")
    dataset_summaries = load_all_summaries(model_family)
    print(f"Loading weights summary for {model_family}")
    weight_summaries = load_weights_summary(model_family)

    neuron_df = weight_summaries["neuron_stats"]
    neuron_df = neuron_df.rename(columns={"neuron_ix": "neuron"}).set_index(
        ["layer", "neuron"]
    )
    neuron_df["weight_norm_penalty"] = (
        neuron_df.input_weight_norm.values**2 + neuron_df.output_weight_norm.values**2
    )
    neuron_df.head()

    try:
        vocab_comps = torch.load(f"data/summary/{model_family}/weights/vocab_comps.pt")
...
def make_corr_compare_df(all_corr_df):
    max_all_corr = all_corr_df.reset_index().groupby(["layer", "neuron"]).max_corr.max()
    mean_all_corr = (
        all_corr_df.reset_index().groupby(["layer", "neuron"]).max_corr.mean()
    )
    min_all_corr = all_corr_df.reset_index().groupby(["layer", "neuron"]).max_corr.min()

    max_all_baseline = (
        all_corr_df.reset_index().groupby(["layer", "neuron"]).baseline.max()
    )
    mean_all_baseline = (
        all_corr_df.reset_index().groupby(["layer", "neuron"]).baseline.mean()
    )
    min_all_baseline = (
        all_corr_df.reset_index().groupby(["layer", "neuron"]).baseline.min()
    )
