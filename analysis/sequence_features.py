...
def get_model_labels(model2spacy, spacy_tag):
    return np.array(
        [get_spacy_tag(tok_ix2spacy_ixs, spacy_tag) for tok_ix2spacy_ixs in model2spacy]
    )
...
def make_spacy_feature_df(model, token_tensor):
    ...
    for i, doc in enumerate(docs):
        ...
        spacy_morph_tense = np.array([t.morph.to_dict().get("Tense", "") for t in doc])
        ...
        gpt2spacy, spacy2gpt = tokenizations.get_alignments(model_tokens, spacy_tokens)
        ...
        morph_number_matrix[i, 1:] = get_model_labels(gpt2spacy, spacy_morph_number)
        morph_person_matrix[i, 1:] = get_model_labels(gpt2spacy, spacy_morph_person)
        morph_prontype_matrix[i, 1:] = get_model_labels(gpt2spacy, spacy_morph_prontype)
        morph_tense_matrix[i, 1:] = get_model_labels(gpt2spacy, spacy_morph_tense)
        morph_verbform_matrix[i, 1:] = get_model_labels(gpt2spacy, spacy_morph_verbform)
        morph_aspect_matrix[i, 1:] = get_model_labels(gpt2spacy, spacy_morph_aspect)
        ...
        is_sent_end_matrix[i, 1:-1] = get_model_labels(gpt2spacy, spacy_end_sent)[:-1]
        is_sent_begins_matrix[i, 1:] = get_model_labels(gpt2spacy, spacy_begin_sent)
    ...
    ent_labels = {f"is_{ent}": ent_type_matrix == ent for ent in ent_label_classes}
    ...
    label_sets = [pos_labels, dependency_labels, morph_labels, ent_labels, misc_labels]
    full_label_dict = {
        k: v.flatten() for label_set in label_sets for k, v in label_set.items()
    }
