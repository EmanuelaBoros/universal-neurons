...
DATASET_ALIASES = {
    ...
    "hipe": "../world-models/data/entity_datasets/hipe-fr.csv",
}
...
def get_hipe_split(split="all"):
    df = pd.read_csv(DATASET_ALIASES["hipe"])
...
def tokenize_hipe_subsets(hipe_ds, model, ctx_len=512):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hf_dataset_split", default="test")
    parser.add_argument("--ctx_len", default=512, type=int, help="Context length")
    parser.add_argument("--n_seq", default=-1, type=int, help="Number of sequences")
    parser.add_argument(
    ...
    # get data
    if args.hf_dataset == "hipe":
        dataset = get_hipe_split(split="all")
    elif args.hf_dataset in DATASET_ALIASES:
    ...
    if args.hf_dataset == "EleutherAI/pile":
    ...
    elif args.hf_dataset == "hipe":
        ds_dict = tokenize_hipe_subsets(dataset, model, ctx_len=512)
        for subset, sub_ds in ds_dict.items():
        ...
        token_dataset = tokenize_and_concatenate(
            dataset, model.tokenizer, max_length=args.ctx_len, column_name="sentence"
        )
