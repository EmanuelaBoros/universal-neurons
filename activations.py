...
def process_masked_layer_activation_batch(
    batch_activations, batch_mask, activation_aggregation
):
    if activation_aggregation is None:
        ...
        processed_activations = masked_activations.sum(dim=1) / seq_indices[:, None]
...
def get_layer_activations(args, model, dataset, device):
    ...
    index_mask = (
        None if "index_mask" not in dataset.column_names else dataset["index_mask"]
    )

    # preallocate memory for activations
    n, d = dataset["tokens"].shape
    n_layers = model.cfg.n_layers
    activation_rows = n if args.activation_aggregation is not None else n * d
    layer_activations = {
    ...
    hooks = [
        (f"blocks.{layer_ix}.{args.activation_location}", save_layer_activation_hook)
    ...
    for step, batch in enumerate(tqdm.tqdm(dataloader)):
        ...
        for lix, (hook_pt, _) in enumerate(hooks):
            ...
            layer_activations[lix][
                layer_offset : layer_offset + pbs
            ] = batch_activations
...
def get_neuron_activations(args, model, dataset, device, neuron_subset):
    ...
    layer_neurons = {
        l: sorted([n for l_, n in neuron_subset if l_ == l]) for l in unique_layers
    }
    # preallocate memory for activations
    n, d = dataset["tokens"].shape
    activation_rows = n * d
    ...
    for step, batch in enumerate(tqdm.tqdm(dataloader)):
        ...
        for hook_ix, (hook_pt, _) in enumerate(hooks):
            ...
            batch_activations = model.hook_dict[hook_pt].ctx["activation"]
            batch_activations = einops.rearrange(batch_activations, "b d n -> (b d) n")
            n_acts = batch_activations.shape[0]
            layer_activations[layer_ix][
                act_offset : act_offset + n_acts
            ] = batch_activations
...
def load_neuron_subset_csv(model, neuron_subset_file, return_df=False):
    ...
    neuron_subset = [
        (int(lix), int(nix))
        for lix, nix in zip(ndf["layer"].values, ndf["neuron"].values)
    ]
    if return_df:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--activation_location", default="mlp.hook_pre", help="Model component to save"
    )
    ...
    # saving arguments
    parser.add_argument("--save_precision", default=8, type=int)
    parser.add_argument("--output_dir", default="cached_activations")
    ...
    if args.neuron_subset is not None or args.neuron_subset_file is not None:
        if args.neuron_subset is not None:
            ...
            neuron_subset = load_neuron_subset_csv(args.model, args.neuron_subset_file)
        print(neuron_subset)
        get_neuron_activations(args, model, tokenized_dataset, device, neuron_subset)
