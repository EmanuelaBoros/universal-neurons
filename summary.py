...
def summarize_activations(args, model, dataset, device):
    ...
    neuron_bin_counts = torch.zeros(n_layers, d_mlp, args.n_bins + 1, dtype=torch.int32)
    ...
    for step, batch in enumerate(tqdm.tqdm(dataloader)):
        ...
        pre_acts = torch.stack(
            [model.hook_dict[hook_pt].ctx["activation"] for hook_pt in pre_hooks], dim=2
        )
    ...
    torch.save(neuron_bin_counts, os.path.join(save_path, "neuron_bin_counts.pt"))
    torch.save(neuron_bin_edges, os.path.join(save_path, "neuron_bin_edges.pt"))
    torch.save(
        neuron_max_activating_index,
        os.path.join(save_path, "neuron_max_activating_index.pt"),
    )
    torch.save(
        neuron_max_activating_value.to(torch.float16),
        os.path.join(save_path, "neuron_max_activating_value.pt"),
    )
    ...
    if args.top_vocab_k_truncate > 0:
        ...
        torch.save(
            top_vocab_avg.to(torch.float16),
            os.path.join(save_path, "neuron_vocab_mean.pt"),
        )
        np.save(os.path.join(save_path, "neuron_vocab_mean_ixs.npz"), top_vocab_avg_ixs)
        torch.save(
            top_vocab_max.to(torch.float16),
            os.path.join(save_path, "neuron_vocab_max.pt"),
        )
        np.save(os.path.join(save_path, "neuron_vocab_max_ixs.npz"), top_vocab_max_ixs)
        torch.save(vocab_counts, os.path.join(save_path, "vocab_counts.pt"))
    elif args.top_vocab_k_truncate == 0:  # don't save
        pass
    else:  # save all (-1)
        torch.save(neuron_vocab_max, os.path.join(save_path, "neuron_vocab_max.pt"))
        torch.save(
            (neuron_vocab_sum / vocab_counts).to(torch.float16),
            os.path.join(save_path, "neuron_vocab_mean.pt"),
        )
        torch.save(vocab_counts, os.path.join(save_path, "vocab_counts.pt"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--token_dataset", help="Name of cached feature dataset")
    parser.add_argument("--output_dir", default="summary")
    parser.add_argument("--batch_size", default=32, type=int)
    parser.add_argument("--n_bins", default=256, type=int)
    parser.add_argument(
        "--top_k_dataset_examples",
        default=50,
        type=int,
        help="Number of top dataset examples to save",
    )
    parser.add_argument(
        "--top_vocab_k_truncate",
        default=100,
        type=int,
