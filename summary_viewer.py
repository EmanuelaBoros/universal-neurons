...
def load_all_summaries(model_name):
    dataset_summaries = {}
    for dataset_name in HIPE_DATASETS:
...
def load_weights_summary(model_name):
    data_dir = os.path.join("data", "summary", model_name, "weights")
    weights_data = {}
    for filename in os.listdir(data_dir):
        if filename.endswith(".pt"):
            weights_data[filename[:-3]] = torch.load(os.path.join(data_dir, filename))
        elif filename.endswith(".npz"):
            weights_data[filename[:-4]] = np.load(os.path.join(data_dir, filename))
        elif filename.endswith(".csv"):
            weights_data[filename[:-4]] = pd.read_csv(os.path.join(data_dir, filename))
    return weights_data
...
def plot_activation_boxplot_by_datasubset(subset_bin_counts, bin_edges):
    ...
    for i, (dataset_name, bin_counts) in enumerate(subset_bin_counts.items()):
        ...
        for percentile in np.concatenate([base_percentiles, percentiles]):
            idx = np.searchsorted(cumulative_counts, percentile / 100.0 * total)
    ...
    ax.bxp(
        stat_dicts, showfliers=False, medianprops={"color": "black", "linestyle": "--"}
    )
    ...
    for ix, p in enumerate(percentiles):
        ax.scatter(
            np.arange(len(stat_dicts)) + 1,
            [d[str(p)] for d in stat_dicts],
            color=colors[ix],
            label=str(p) + "%",
            marker="_",
            s=250,
...
def plot_activation_distributions(subset_bin_counts, neuron_bin_edges):
    ...
    ax = axs[0]
    ax.hist(bin_edges, weights=total_bin_counts, bins=len(bin_edges), log=True)
...
def plot_activation_distributions_plotly(subset_bin_counts, neuron_bin_edges):
    ...
    for dataset_name, bin_counts in subset_bin_counts.items():
        fig.add_trace(
            go.Scatter(x=bin_edges, y=bin_counts / bin_counts.sum(), name=dataset_name),
            row=1,
            col=2,
        )
    ...
    fig.update_xaxes(range=[bin_edges[min_bin], bin_edges[max_bin]], row=1, col=1)
    fig.update_xaxes(range=[bin_edges[min_bin], bin_edges[max_bin]], row=1, col=2)
    fig.update_xaxes(range=[bin_edges[min_bin], bin_edges[max_bin]], row=1, col=3)
...
def get_vocab_summary_dfs(dataset_summaries, decoded_vocab, layer, neuron):
    vocab_col_names = [
        "neuron_vocab_max",
        "neuron_vocab_max_ixs",
        "neuron_vocab_mean",
        "neuron_vocab_mean_ixs",
    ]
...
def vocab_heatmap(value_df, token_df, display_top_k=25, max=True):
    ...
    fig.update_xaxes(
        ticktext=value_df.columns,
        tickvals=np.arange(len(value_df.columns)),
        tickangle=-45,
    )
    fig.show()
    return fig
...
def make_vocab_line_plot(max_value_df, max_token_df, mean_value_df, mean_token_df):
    ...
    fig = sp.make_subplots(
        ...
        shared_yaxes=True,
        subplot_titles=(
            "Max Activating Vocab by Dataset",
    ...
    for i, col in enumerate(max_value_df.columns):
        fig.add_trace(
            ...
            row=1,
            col=1,  # This goes to the left subplot
        )
    ...
    for i, col in enumerate(mean_value_df.columns):
        fig.add_trace(
            ...
            row=1,
            col=2,  # This goes to the right subplot
        )
    # Update layout
    fig.update_layout(xaxis_title="token rank", yaxis_title="Token Activation")
    fig.show()
    return fig
...
def display_max_activating_examples(
    ...
    layer,
    neuron,
    display_k_per_dataset=5,
    tokens_to_display_before=20,
    tokens_to_display_after=5,
):

    for dataset_name, tokenized_dataset in token_datasets.items():
        display(Markdown("### " + dataset_name))
        neuron_max_ix = dataset_summaries[dataset_name]["neuron_max_activating_index"][
            layer, neuron
        ].numpy()
        neuron_max_value = dataset_summaries[dataset_name][
            "neuron_max_activating_value"
        ][layer, neuron].numpy()
        ...
        for i in range(display_k_per_dataset):
            ...
            display_tokens = tokenized_dataset[
                neuron_max_seq_ix[i], display_min:display_max
            ].numpy()
            display_str = "".join(
                [
                    (
                        decoded_vocab[t]
                        if ix != tokens_to_display_before
...
def neuron_or_vocab_composition_heatmap(
    top_value_df,
    top_ix_df,
    bottom_value_df,
    bottom_ix_df,
    ...
    fig = sp.make_subplots(
        rows=1, cols=2, subplot_titles=("max similarity", "min similarity")
    )
    ...
    heatmap_max = go.Heatmap(
        ...
        textfont={"size": 10},
        colorscale="Inferno",
        colorbar=dict(len=0.5, yanchor="bottom", y=0.5, title="Max composition"),
    )
    ...
    # add tick labels with rotation
    fig.update_xaxes(
        ticktext=top_value_df.columns,
        tickvals=np.arange(len(top_value_df.columns)),
        tickangle=-45,
    )
...
def neuron_and_vocab_density_plots(weights_data, layer, neuron):
    ...
    dummy_bin = np.array([bin_edges[-1] + bin_edges[-1] - bin_edges[-2]])
    ...
    for comp_type, legend_key in vocab_comp_type_display_names.items():
        bin_counts = weights_data["vocab_comps"][comp_type]["comp_hist"][
            layer, neuron
        ].numpy()
    ...
    axs[0].set_xlim(bin_edges[vocab_min_bin] - 0.025, bin_edges[vocab_max_bin] + 0.025)
    axs[1].set_xlim(bin_edges[vocab_min_bin] - 0.025, bin_edges[vocab_max_bin] + 0.025)
    axs[1].set_yscale("log")

    neuron_min_bin = len(bin_edges) - 1
    neuron_max_bin = 0
    for comp_type, legend_key in neuron_comp_type_display_names.items():
        bin_counts = weights_data["neuron_comps"][comp_type]["comp_hist"][
            layer, neuron
        ].numpy()
        comp_bins = np.nonzero(bin_counts)[0]
    ...
    axs[2].set_xlim(
        bin_edges[neuron_min_bin] - 0.025, bin_edges[neuron_max_bin] + 0.025
    )
    axs[3].set_xlim(
        bin_edges[neuron_min_bin] - 0.025, bin_edges[neuron_max_bin] + 0.025
    )
    ...
    ax_titles = [
        "vocab composition",
        "vocab composition (log)",
        "neuron composition",
        "neuron composition (log)",
    ]
...
def plot_neuron_attn_composition(weights_data, layer, neuron):
    ...
    fig = sp.make_subplots(rows=1, cols=4, subplot_titles=TITLES, shared_yaxes=True)

    for i, attn_comp in enumerate(ATTN_COMPS):
        fig.add_trace(
            go.Heatmap(z=weights_data[attn_comp][layer, neuron], coloraxis="coloraxis"),
            row=1,
            col=i + 1,
        )
    ...
    fig.update_layout(coloraxis=dict(colorscale="Viridis"), width=1000, height=350)
    fig.show()
...
def display_summary(
    ...
    make_vocab_line_plot(max_value_df, max_token_df, mean_value_df, mean_token_df)
